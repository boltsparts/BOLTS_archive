...
- Has extensive [Documentation]({{ url(docs.index) }}) that describes how to [contribute]({{ url(contribute) }}) new parts.
...
[contribute to BOLTS]({{ url(contribute) }}) by trying it and providing
feedback and ideas for improvement or by adding more parts.

Have fun with [this release]({{ url(downloads) }})
