...
directory contains at least a "data" and a "drawings" directory, plus
optionally a number of backend directories. See list-of-backends_ for details.

.. This directory layout permits to keep collections in git submodules, but I
   do not like the alternatives that would allow to. Plus I am not sure how
   useful it is to keep collections independently from each other.
...
Each backend directory contains at least an output directory, where the
distribution_ of files generated by the backend is stored. Additional
directories can be present, this is specified by the backend_.
...
A backend is process that uses backend independent data and backend specific
...
about parts, their geometries and dimensions together with number of templates
and stylesheets and produces a number of HTML pages with a nicely rendered
description of the parts. Other backends could produce data that is suitable
for specific CAD applications.
...
should be made optional, to keep every backend functional independent of other
backends.
