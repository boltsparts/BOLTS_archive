...
from .errors import ModuleNameCollisionError
...
class OpenSCADBackend(Backend):
    ...
    def write_output(self, out_path, **kwargs):
        args = self.validate_arguments(
            kwargs,
            ["target_license", "version"],
            {"stable": False, "expand": False}
        )
        ...
        for filename in listdir(join(self.repo.path, "backends", "openscad")):
            if not filename.endswith(".scad"):
                continue
            copy(
                join(self.repo.path, "backends", "openscad", filename),
        ...
        if args["stable"]:
            ...
            version_fid.write(
                ...
                (major, minor, args["target_license"])
            )
        else:
            version_fid.write('function BOLTS_version() = "%s";\n' % args["version"])
        date = datetime.now()
        version_fid.write(
            'function BOLTS_date() = [%d,%d,%d];\n' %
        ...
        for scadfile, in self.dbs["openscad"].iterscadfiles():
            ...
            copy(
                join(self.dbs["openscad"].backend_root, scadfile.path),
                join(out_path, "base", scadfile.path)
            )
            # include files
            if not args["expand"]:
            ...
            # process classes
            for cl, module in self.dbs["openscad"].iterclasses(
                ["class", "module"], filter_scadfile=scadfile
            ):
    ...
    def write_classfile(self, fid, cl, module):
        ...
        for table, i in zip(params.tables2d, range(len(params.tables2d))):
            ...
            for col, j in zip(table.columns, range(len(table.columns))):
                fid.write(
                    'colidx == "%s" ? %s_table2d_rows_%d(rowidx)[%d] :\n' % (col, cl.id, i, j)
                )
            fid.write('"Error";\n\n')
        ...
        if self.dbs["openscad"].module_connectors.contains_src(module):
            ...
            call = "%s(%s)" % (
                connectors.name, ", ".join(cargs[arg] for arg in connectors.arguments)
            )
        ...
        for std, in self.dbs["openscad"].iterstandards(filter_class=cl):
            ...
            for pname in params.free:
                fid.write(
                    '\tBOLTS_check_parameter_type("%s","%s",%s,"%s");\n' %
            ...
            # dims
            fid.write(
                'function %s_dims(%s) = %s_dims(%s);\n\n' %
                (std.get_id(), get_signature(params), cl.id, get_signature(params, False))
            )

            # conns
            fid.write(
                'function %s_conn(location,%s) = %s_conn(location,%s);\n\n' %
                (std.get_id(), get_signature(params), cl.id, get_signature(params, False))
            )
